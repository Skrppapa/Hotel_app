...
def rooms_ids_for_booking(
    ...
    """with rooms_count as (
               ...
               where date_from <= '2026-10-07' and date_to >= '2026-08-01'
               group by room_id
           ),
           rooms_left_table as (
    ...
    # Это не cte это просто подзапрос вынесенный в переменную, он используется в cte ниже
    rooms_ids_for_hotel = (
        select(RoomsOrm.id)
        .select_from(RoomsOrm)
    )
    ...
    rooms_ids_for_hotel = (
        rooms_ids_for_hotel
        .subquery(name="rooms_ids_for_hotel") # Явное обозначение, что это подзапрос
    )
